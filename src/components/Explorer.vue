  ...
  <div class="relative flex-auto flex flex-col overflow-hidden">
    ...
    <div class="absolute">
      <div ref="dragImage"  class="absolute -z-50 -top-96">
        ...
        <div class="text-neutral-700 dark:text-neutral-300 p-1 absolute text-center top-4 right-[-2rem] md:top-5 md:right-[-2.4rem] z-20 text-base">{{ selectedCount }}</div>
      </div>
    </div>
    ...
    <div
        ...
        class="h-full w-full text-base vf-selector-area min-h-[150px] overflow-auto p-1 z-0"
        ref="selectorArea">
      ...
      <div draggable="true"
          ...
          <div>
            <div class="relative">
              ...
              <div class="absolute hidden md:block top-1/3 w-full text-center text-neutral-500" v-if="!(item.mime_type ?? '').startsWith('image') && item.type != 'dir'">{{ ext(item.extension) }}</div>
            </div>
