<template>
  <v-f-modal-layout>
    <div class="sm:flex sm:items-start">
      <div class="mx-auto flex-shrink-0 flex items-center justify-center h-10 w-10 rounded-full bg-indigo-200 dark:bg-gray-500 sm:mx-0 sm:h-10 sm:w-10">
        <svg xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6 stroke-indigo-500 dark:stroke-indigo-100" fill="none" viewBox="0 0 24 24" stroke="none" stroke-width="1.5">
      ...
      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
        ...
        <div class="mt-2">
          <p class="text-sm text-gray-500 mb-2">{{ t('Create a new file') }}</p>
    ...
    <template v-slot:buttons>
      <button type="button" @click="createFile" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-500 dark:bg-gray-700 dark:hover:bg-gray-600/75 text-base font-medium text-white hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
        {{ t('Create') }}
      </button>
